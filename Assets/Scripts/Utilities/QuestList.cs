...
public static class QuestList
{
    ...
    public static Quest GetQuestFromData(JsonData q)
    {
        ...
        quest.failOnDeath = q.ContainsKey("Fail On Death") ? (bool)q["Fail On Death"] : false;
    ...
    static List<QuestEvent> GetEvents(JsonData data)
    {
        ...
        for (int i = 0; i < data["Events"].Count; i++)
        {
            ...
            if (d.ContainsKey("Remove Spawns"))
            {
                QuestEvent.SubEvent rs = new QuestEvent.SubEvent("Remove Spawns", "", null, 0);
                newEvent.SubEvents.Add(rs);
            }
