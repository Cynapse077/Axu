...
public static class TraitList {
    ...
    public static void FillTraitsFromData() {
		...
		string listFromJson = File.ReadAllText(Application.streamingAssetsPath + "/Data/Entity Data/Traits.json");
		...
		listFromJson = File.ReadAllText(Application.streamingAssetsPath + "/Data/Entity Data/Wounds.json");
	...
	public static Wound GetWoundFromData(JsonData wData) {
		...
		w.Desc = wData["Description"].ToString();
	...
	public static Trait GetTraitFromData(JsonData tData) {
		...
		t.description = tData["Description"].ToString();
		...
		if (tData.ContainsKey("Replace_Body_Part")) {
			...
			t.replaceBodyPart.removeAll = newData.ContainsKey("RemoveAll") ? (bool)newData["RemoveAll"] : false;
