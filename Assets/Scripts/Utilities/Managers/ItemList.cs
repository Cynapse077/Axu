...
public static class ItemList {
    ...
    public static void CreateItems() {
		...
		noneItem.ID = "none";
		noneItem.lootable = false;
        noneItem.itemType = Proficiencies.Unarmed;
        noneItem.rarity = 100;
        noneItem.flavorText = "There is nothing here.";
		...
		if ((i.HasProp(ItemProperty.Weapon) || i.HasProp(ItemProperty.Armor) || i.HasProp(ItemProperty.Ranged)) && !i.stackable && i.lootable) {
			...
			if (SeedManager.combatRandom.Next(100) <= threshold) {
				...
				if (!newItem.ContainsProperty(p)) {
					...
					if (p == ItemProperty.Shock_Nearby) {
						CLuaEvent ev = new CLuaEvent("OnHit", "ItemActions", "ShockAdjacent", "");
						newItem.components.Add(ev);
					} else if (p == ItemProperty.DrainHealth) {
						CLuaEvent ev = new CLuaEvent("OnHit", "ItemActions", "DrainHealth", "");
						newItem.components.Add(ev);
					}
			...
			if (SeedManager.combatRandom.Next(100) <= threshold) {
				string[] sk = new string[] {
					"juke", "charge", "energize", "firestream",
					"icestream", "radblast", "firewall",
					"confsquare", "blink", "sprint"
				};

				if (newItem.GetItemComponent<CAbility>() == null) {
	...
	public static Item GetItemByID(string id) {
		if (id == "none" || id == "None" || id == GetNone().ID)
			...
			if (newItem == null || !newItem.HasComponent<CLiquidContainer>())
				return null;
			
			CLiquidContainer cl = newItem.GetItemComponent<CLiquidContainer>();
    ...
    public static Item GetItemByRarity(int rar) {
		...
		if (item.HasProp(ItemProperty.Ammunition))
			item.amount = SeedManager.combatRandom.Next(1, 11);
	...
	public static void TryAddMod(Item i, int chance) {
    	if (SeedManager.combatRandom.Next(100) <= chance) {
			List<ItemModifier> ms = modifiers.FindAll(m => !m.unique && m.CanAddToItem(i));
    ...
    public static Item GetSeveredBodyPart(BodyPart part) {
        if (part.slot == ItemProperty.Slot_Head)
            return GetItemByID("severed_head");
        if (part.slot == ItemProperty.Slot_Arm)
            return GetItemByID("severed_arm");
        if (part.slot == ItemProperty.Slot_Leg)
            return GetItemByID("severed_leg");
        if (part.slot == ItemProperty.Slot_Tail)
            return GetItemByID("severed_tail");
        if (part.slot == ItemProperty.Slot_Wing)
            return GetItemByID("severed_wing");
		
		return GetItemByID("fleshraw");
    }
    ...
    static List<Item> ItemsFromData() {		
		...
		if (string.IsNullOrEmpty(jsonString))
			Debug.LogError("Item List null.");

		if (string.IsNullOrEmpty(jsonString2))
			Debug.LogError("Natural Items null.");

		List<Item> itemList = GetFromData(data, "Items");
		List<Item> natItemList = GetFromData(data2, "Natural Items");
		for (int i = 0; i < natItemList.Count; i++) {
			itemList.Add(natItemList[i]);
		}
    ...
    static List<ItemModifier> ModifiersFromData() {
        ...
        for (int i = 0; i < data["Mods"].Count; i++) {
            ...
            m.name = newData["Name"].ToString();
			m.ID = newData["ID"].ToString();
            m.description = newData["Description"].ToString();
    ...
    public static List<Item> GetFromData(JsonData data, string listName) {
		...
		for (int i = 0; i < jsonItemList.Count; i++) {
			...
			if (currentItem.ContainsKey("Components"))
				it.components = GetComponentsFromData(currentItem["Components"]);
	...
	static List<MapObjectBlueprint> FillMOBlueprintList() {
		...
		for (int i = 0; i < data["Objects"].Count; i++) {
			...
			mob.Name = newData["Name"].ToString();
			mob.objectType = newData["ObjectType"].ToString();

			mob.spriteID = newData["Sprite"].ToString();
			mob.description = newData["Description"].ToString();
            ...
            if (newData.ContainsKey("OnInteract")) {
                LuaCall lc = new LuaCall(newData["OnInteract"]["File"].ToString(), newData["OnInteract"]["Function"].ToString());
                mob.onInteract = lc;
            }
	...
	static List<Liquid> GetLiquidsFromData() {
		...
		for (int i = 0; i < data["Liquids"].Count; i++) {
			JsonData d = data["Liquids"][i];
			List<CLuaEvent> ev = new List<CLuaEvent>();
			...
			int addictiveness = d.ContainsKey("Addictiveness") ? (int)d["Addictiveness"] : 0;
			byte r = (byte)((int)d["Color"][0]), g = (byte)((int)d["Color"][1]), b = (byte)((int)d["Color"][2]);
			Color32 col = new Color32(r, g, b, 255);
			Liquid newliq = new Liquid(d["ID"].ToString(), d["Name"].ToString(), d["Description"].ToString(), 101 - (int)d["Frequency"], (int)d["Price Per Unit"], addictiveness, ev, col);

			liquidList.Add(newliq);
		}

		if (data.ContainsKey("Mixing Tables")) {
			Liquid.mixingOutputs = new List<Liquid.MixingOutput>();

			for (int i = 0; i < data["Mixing Tables"].Count; i++) {
				Liquid.MixingOutput mo = JsonMapper.ToObject<Liquid.MixingOutput>(data["Mixing Tables"][i].ToJson());
				Liquid.mixingOutputs.Add(mo);
			}
		}
