...
public class Console : MonoBehaviour {
	...
	void OnGUI() {
		if (UserInterface.showConsole) {
			...
			Rect consoleOutline = new Rect(5, 5, 420, 270);
			GUI.Box(consoleOutline, "");
			GUI.Label(new Rect(110, 5, 200, 25), "<b><color=#ffa500ff>[ CONSOLE ]</color></b>");
			MyConsole.DrawConsole(new Rect(15, 30, 400, 200));
			GUI.SetNextControlName("TextField");
			textField = GUI.TextField(new Rect(15, 235, 350, 25), textField);
			GUI.FocusControl("TextField");
			if (GUI.Button(new Rect(367, 235, 50, 25), "Enter")) {
				SendToParse();
			}
	...
	public void ParseTextField(string textToParse) {
		...
		if (parsedText[0] == "set") {
			...
			if (parsedText.Length < 2)
				...
				} else if (parsedText[1] == "hp" || parsedText[1] == "HP") {
					playerStats.health = playerStats.maxHealth = o;
				} else if (parsedText[1] == "st" || parsedText[1] == "ST") {
		...
		if (parsedText[0] == "revealall" || parsedText[0] == "reveal" || parsedText[0] == "revealmap") {
		...
		if (parsedText[0] == "restart") {
		    SceneManager.LoadScene(0);
			return;
		}
		if (parsedText[0] == "wizard") {
		...
		if (parsedText[0] == "spawn") {
			if (parsedText.Length < 2) {
				MyConsole.Error("No object specified.");
				return;
			}
			if (parsedText.Length < 3) {
				MyConsole.Error("Spawn needs coordinates.\nEg: '3 -5' for spawning +3 tiles away on x axis, and -5 tiles away on y axis.");
				return;
			}
			if (parsedText.Length < 4 && parsedText[2] != "random") {
				MyConsole.Error("Invalid coordinates. Use a space between the x and y numbers.");
				return;
			}
			TileMap tileMap = World.tileMap;
			Coord wp = tileMap.WorldPosition;
			int x = int.Parse(parsedText[parsedText.Length - 2]), y = int.Parse(parsedText[parsedText.Length - 1]);
			Coord lp = (parsedText[parsedText.Length - 1] == "random") ? tileMap.GetRandomPosition() : new Coord(playerEntity.posX + x, playerEntity.posY + y);
            ...
            //Spawn NPCs
            string spawnName = parsedText[1];
			int o = 0;
			for (int e = 1; e < parsedText.Length; e++) {
				if (int.TryParse(parsedText[e], out o)) {
					if (e == parsedText.Length - 2)
						lp.x = playerEntity.posX + o;
					if (e == parsedText.Length - 1)
						lp.y = playerEntity.posY + o;

					NPC n = EntityList.GetNPCByName(spawnName, wp, lp);
					if (n != null) {
						World.objectManager.SpawnNPC(n);
						MyConsole.NewMessage("    " + spawnName + " spawned at (" + lp.x + "," + lp.y + ") relative to player.");
						tileMap.CheckNPCTiles();
					}

					return;
				} else if (e > 1)
					spawnName += " " + parsedText[e];
			}
			return;
        }

        if (parsedText[0] == "hunger") {
			if (parsedText.Length < 2) {
				MyConsole.Error("Specify an amount to set hunger to.");
				return;
			}

			playerStats.Hunger = int.Parse(parsedText[1]);
            MyConsole.NewMessage("    Player hunger set.");
            return;
        }
		
		if (parsedText[0] == "godmode" || parsedText[0] == "gm") {
		...
		if (parsedText[0] == "explore") {
			...
			Manager.noHunger = toggleOn;
			MyConsole.NewMessage("    Encounters and hunger " + (toggleOn ? "disabled" : "enabled"));
			return;
		}
		
		if (parsedText[0] == "fov" || parsedText[0] == "lighting") {
			if (parsedText.Length < 2 || parsedText.Length > 2) {
				MyConsole.Error("Invalid. 0 for off, 1 for on.");
				return;
			}
    ...
    //TRAVEL
       ...
       //TRAITS
        ...
        if (parsedText[0] == "randommutation") {
			playerStats.Radiate(100);
            return;
        }

		if (parsedText[0] == "givetrait") {
            if (parsedText.Length < 2) {
		...
		if (parsedText[0] == "giveability" || parsedText[0] == "giveskill") {
			if (parsedText.Length < 2) {
		...
		if (parsedText[0] == "radiate") {
			playerStats.Mutate();
			return;
		}

        if (parsedText[0] == "mutate") {
            if (parsedText.Length < 2) {
		...
		if (parsedText[0] == "give" || parsedText[0] == "grant") {
			if (parsedText.Length < 2) {
		...
		if (parsedText[0] == "multigive" || parsedText[0] == "multigrant") {
			if (parsedText.Length < 2) {
				MyConsole.Error("Need to provide an item name.");
				return;
			}
		...
		if (parsedText[0] == "killme") {
			playerStats.StatusEffectDamage(1000, DamageTypes.Blunt);
			return;
		}
		if (parsedText[0] == "injure" || parsedText[0] == "hurt") {
			if (parsedText.Length < 2) {
			...
			MyConsole.NewMessage("Damaged the player by " + damage + ".");
			return;
		}

		if (parsedText[0] == "givestatus") {
			if (parsedText.Length < 2) {
		...
		if (parsedText[0] == "fullheal" || parsedText[0] == "heal") {
			playerStats.Heal(playerStats.maxHealth);
			MyConsole.NewMessage("    Fully healed player.");
			return;
		}
		if (parsedText[0] == "detonate" || parsedText[0] == "killall") {
			GameObject[] entities = GameObject.FindGameObjectsWithTag("Entity");
			for (int i = 0; i < entities.Length; i++) {
		...
		if (parsedText[0] == "?" || parsedText[0] == "help" || parsedText[0] == "commands") {
			...
			MyConsole.NewHelpLine("setpos [x] [y]", "Travel to a specific destination. Constraints: 0-199 on each axis.");
			MyConsole.NewMessage("  -<b>godmode</b> <i>[0-1]</i>\n      [0] = off\n      [1] = on");
			MyConsole.NewMessage("  -<b>fov</b> <i>[0-1]</i>\n      Whether to show fog of war or not. \n0 = off  1 = on");
			MyConsole.NewMessage("  -<b>explore</b> <i>[0-1]</i>\n      [0] = off\n      [1] = on\n      Enables or disables map encounters and the hunger clock.");
			MyConsole.NewMessage("  -<b>gold</b> <i>[amount]</i>\n      Gives [amount] gold to the player.");
			MyConsole.NewMessage("  -<b>spawn</b> <i>[name] [x] [y]</i>\n      Spawns an item or NPC. \n" +
				"          [name] = \"chest\", \"resource\", \"cryopod\", \"bookshelf\", \"statue\", \"crystal\", or Any NPC name.");
			...
			MyConsole.NewMessage("  -<b>modwep</b> <i>[mod ID]</i>\n    Modifies the first non-severed hand's equipped weapon with the selected ID.");
			...
			MyConsole.NewMessage("  -<b>hunger <i>[amount (0-8000)]</i></b>\n      Sets the satiation to the number specified.");
			MyConsole.NewMessage("  -<b>completequest</b>\n    Completes the current tracked quest.");
			MyConsole.NewMessage("  -<b>startquest</b>\n    Starts the quest with the input ID.");
