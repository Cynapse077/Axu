...
public class ItemActionsPanel : MonoBehaviour {
	...
	public void Display(Item item, Inventory inv) {
		...
		for (int i = 0; i < numActions; i++) {
			GameObject g = (GameObject)Instantiate(ActionButton, transform);
	...
	void SelectPressed(int index) {
		...
		if (actionName == "Throw") {
			if (curInv == ObjectManager.player.GetComponent<Inventory>()) {
		...
		if (actionName == "Use" && curInv == ObjectManager.player.GetComponent<Inventory>()) {
			if (item.HasProp(ItemProperty.Blink) || item.HasProp(ItemProperty.Teleport) || item.HasProp(ItemProperty.Surface_Tele))
				World.userInterface.CloseWindows();
		}
